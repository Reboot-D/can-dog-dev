...
export class NotificationService {
     ...
     */
    async getEventsForNotification() {
        const now = new Date();
      ...
      `)
        ...
        const transformedEvents = (events || []).map(event => ({
            ...
            pet: {
                name: event.pet?.name || 'Unknown Pet',
                breed: event.pet?.breed || null,
            },
            user: {
                email: event.user?.email || '',
                full_name: event.user?.full_name || null,
            }
        }));
     ...
     */
    async markEventAsNotified(eventId) {
