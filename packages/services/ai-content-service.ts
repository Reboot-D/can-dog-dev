...
export class AIContentService {
  ...
  private config: AIContentServiceConfig;
   ...
   */
  async generateNotificationContent(context: PetEventContext): Promise<AINotificationContent> {
    try {
   ...
   */
  private createNotificationPrompt(context: PetEventContext): string {
...
- 内容要符合中国文化背景和表达习惯`;
   ...
   */
  private parseGeneratedContent(generatedText: string, context: PetEventContext): Omit<AINotificationContent, 'success' | 'error'> {
    try {
   ...
   */
  private createFallbackContent(context: PetEventContext): Omit<AINotificationContent, 'success' | 'error'> {
    return {
  ...
  private createFallbackSubject(context: PetEventContext): string {
    return `${context.petName}的${context.eventTitle}提醒`;
  }

  private createFallbackReminder(context: PetEventContext): string {
    const userName = context.userName || '主人';
    return `亲爱的${userName}，这是一个友好的提醒：${context.petName}有一个即将到来的${context.eventTitle}，预定日期是${context.eventDate}。请记得为您的爱宠做好准备。`;
  }

  private createFallbackTip(context: PetEventContext): string {
   ...
   */
  private validateContent(content: Omit<AINotificationContent, 'success' | 'error'>): boolean {
